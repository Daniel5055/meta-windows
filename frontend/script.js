...
server.on('state', (msg) => {
    if (!(msg.id in windows)) {
        ...
        windows[msg.id].el.style.backgroundColor = colors[Object.values(windows).length - 1]
...
function update(other) {
    ...
    console.log('hello', left, right, myself.state.w, other)
    ...
    console.log('hi', target.style.left, target.style.top, target.style.width, target.style.height)
}
